...
| 20 | [Budget — how much of a section's allocation is spoken for, and one estimate column instead of two](20-budget-section-allocation-remaining.md) | **Proposed, not built.** Two halves from one round of feedback on `/budget`, shippable independently. §1–§6: the per-category half of spec 19's wedding-level "allocated 97% · $1,200 unallocated" — one line at the top of each section saying what its lines' percentages add up to (does 38+30+42 make 100? no — 110), in money, and what's left to allocate. §7: collapse spec 19's "Allocated" and "Estimated" figures into one column, since on a line with no typed estimate they are the same number twice. Recommends **no schema at all** — a pure function over rows `/budget` already loads — so its §8 Open Questions gate the whole feature, not just its screens. Question 5 (the target stays on the row as a secondary note) answered 2026-09-20, §11; questions 1-4 still open. | Spec 19, built but unmerged |
