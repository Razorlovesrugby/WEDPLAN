...
| 5 | [Multi-cut guest lines, and a day-of run sheet](05-multi-cut-lines-and-run-sheet.md) | Proposed. Two independent parts (A: generalise the fixed A/B cut lines to any number of planner-named lines; B: a pinned/predecessor day-of schedule). Nothing built — blocked on each part's own Open Questions section. | V1 only |
| 6 | [Budget management](06-budget-management.md) | Proposed. Categories/items on the platform spec's four-number model, a per-unit costing mechanism covering food/alcohol per-head pricing, a live per-head figure on the ranking screen, and a payment-due sync into the existing reminders digest. Nothing built — blocked on its Open Questions section. | V1 only; its reminders sync additionally depends on specs 1 and 2, already shipped |
