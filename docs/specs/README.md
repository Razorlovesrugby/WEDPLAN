...
| 18 | [Budget — GST inclusive/exclusive per line](18-budget-gst.md) | **Proposed, not built — no migration, no screen change.** A per-line `gst_treatment` toggle on `budget_items`; when exclusive, every total that sums the line (its own current figure, `v_budget_summary`, the dashboard tile, `/guests/rank`'s per-head figure) adds a hardcoded 15%. Nothing built until §6's open questions are answered. | Spec 6 / 6.1, already built |
