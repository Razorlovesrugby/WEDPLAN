...
| 21 | [A page of their own — per-household addresses on the wedding site](21-per-household-site-addresses.md) | **Proposed, fully answered (2026-09-20), not built — no migration, no route, no screens.** A readable per-household address (`/w/<wedding-slug>/okonkwo-4f7ak`) becoming the one link per household: the name the planner recognises plus a five-character random suffix, so spec 14 Q1's unguessable credential survives the cosmetic change (§3). Most of the page already exists as `/rsvp/[token]`; new are the address itself, per-event guest-facing notes stitched per household (§5.4), an alias table so an edited slug does not break the link already sent, and the retirement of `/i` and `/rsvp` into 301s. One migration (§7). Build not authorized. | Spec 14, built; V1's `households` / `invitations` / `invitation_events` |
