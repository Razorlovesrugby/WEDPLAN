...
| 14 | [The public wedding site, and the invites that point at it](14-public-site-and-invites.md) | **Steps 0–2 largely built, step 3 about half (2026-09-18); steps 4–5 not started.** All twelve questions answered. `0015_wedding_slug.sql` + `0016_save_the_date.sql`, `/w/[slug]` in the Script theme, the `/site` and `/site/theme` editors, the FAQ starter library, an `.ics` route, a seeded example site, and the `/i/[token]` stationery card with a real Open Graph image plus save-the-date and broadcast senders. 106 new unit tests (412 total). The OG image is the only part seen rather than only tested. **Next: step 4, the coach.** See the spec's "Build status" for the per-file handoff. | V1; spec 9 for the moodboard sections already on `/w` |
