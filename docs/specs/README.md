...
| 14 | [The public wedding site, and the invites that point at it](14-public-site-and-invites.md) | **Steps 0–2 largely built (2026-09-17); steps 3–5 not started.** All twelve questions answered. `0015_wedding_slug.sql`, `/w/[slug]` rendered from `site_content` in the Script theme with self-hosted fonts and a contrast-validated palette, the `/site` and `/site/theme` editors, the FAQ starter library, an `.ics` route, and a seeded example site. 93 new unit tests (399 total), 20 new SQL assertions (187 total). **Next: step 3, the invites** — the only part with a date that cannot move. Never opened in a browser or run against a live project. See the spec's "Build status" for the per-file handoff. | V1; spec 9 for the moodboard sections already on `/w` |
