...
| 22 | [Inviting per event, answering, and knowing they looked](22-per-event-invite-status-and-tracking.md) | **Proposed, fully answered (2026-09-20), not built — no migration, no screens.** Per-event invite status in `/guests`: household default with per-guest overrides (`guest_event_overrides` + `v_guest_event_invites`, so a bulk household change cannot silently re-invite someone deliberately removed), "invite sent" as a real state, clickable cells with a menu rather than a cycle, an event's details shown only to people invited to it — which revises spec 14 §6's list-and-mark rule — one-tap Yes/No from the invitation email with an optional decline note, and every open logged rather than only the first. All six questions answered: Maybe stays, the page never names who is *not* invited, chasing stays per household, no section analytics, the grid stays flat. | V1's guest list, spec 14, spec 21, all built |
| 23 | [The site builder: blocks, widgets, photos and a live preview](23-site-builder-and-widgets.md) | **Proposed, fully answered (2026-09-20), not built — no migration, no screens.** A block builder with a live preview (not a free canvas — it would need a hand-built layout per breakpoint), blocks replacing the twelve fixed sections, draft-then-publish through `site_revisions` keeping the last twenty, one scrolling page personalised in place, and four widget families — photos (the biggest gap: there is no upload anywhere today), music, map/travel, and dress code / FAQ / countdown. All eight questions answered; **Q1 reopens spec 14 §11's third-party ban** — embeds are allowed per block, off by default, and refused on a personalised address without a confirm, because the referrer there is a household's credential. | Spec 14, spec 21, built; spec 22 supplies the who-sees-what rule |
