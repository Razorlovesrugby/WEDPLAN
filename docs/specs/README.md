...
| 6.1 | [Budget — quantity × unit price, and linking to an existing task](06.1-budget-quantity-and-task-linking.md) | Proposed, nothing built. Part A (a manual quantity × unit price costing basis) has open questions awaiting the planner. Part B (searching for and linking an existing task, not just a whole list or a new one) needs no new decision — it was already specified in spec 6 §7 and simply wasn't finished in session 12's build. | Spec 6, already built |
