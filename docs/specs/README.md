...
| 20 | [Budget — how much of a section's allocation is spoken for](20-budget-section-allocation-remaining.md) | **Proposed, not built.** The per-category half of spec 19's wedding-level "allocated 97% · $1,200 unallocated": one line at the top of each section saying what its lines' percentages add up to (does 38+30+42 make 100? no — 110), what that is in money, and what's left to allocate. Recommends **no schema at all** — a pure function over rows `/budget` already loads — so its §7 Open Questions gate the whole feature, not just its screens. | Spec 19, built but unmerged |
