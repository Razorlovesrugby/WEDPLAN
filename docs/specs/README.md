...
| 14 | [The public wedding site, and the invites that point at it](14-public-site-and-invites.md) | **Proposed, partly answered (2026-09-17), not built.** A discovery pass against [aisle.wedding](https://aisle.wedding), requested by the planner. The reference site could not be read (egress policy blocks that host — see the spec's §0), so its design detail is still owed: screenshots or an allowlist entry. Four questions answered in session 20 — household token kept, **local wedding with a coach** (room blocks and airports cut), link-out payments, Script theme — cutting roughly a third of the scope and leaving build steps 1–3 with no migration at all. The remaining §14.2 questions still gate everything, and Q12 (a web licence for the script typeface) blocks step 1. | V1; spec 9 for the moodboard sections already on `/w` |
...
answers, steps 1–3 (theme and renderer, schedule and FAQ, and the whole
invitation surface) need **no migration at all** and can ship while §14.2 is
still outstanding. Step 3 is the only part carrying a date that cannot move,
so it goes first if the send is close.
