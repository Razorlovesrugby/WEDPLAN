...
| 19 | [Budget — an overall budget, percentage allocations, and allocation-derived estimates](19-budget-allocation-percentages.md) | **Decided, not built — no migration, no screens.** All six open questions answered 2026-09-20 (the planner took the recommendation on every one; see §12), but answering them is not authorization to build — nothing starts until the planner says to build it. Top-down budgeting on top of spec 6's bottom-up model: `weddings.total_budget`, an `allocation_pct` on both `budget_categories` and `budget_items`, a per-category rollup of target vs. current with the over/under in dollars and percent, and a line's allocation standing in as its estimate until one is typed. One residual item inside decision 6: §8's suggested percentage table is in, but its numbers are a placeholder still wanting the planner's eye. | Spec 6 / 6.1 and spec 18, already built |
