...
| 4 | [Household editing and moving guests between households](04-household-management.md) | **Built and merged** — `moveGuest`/`moveGuests` actions, `HouseholdPicker`, wired into the household page, guest page, and guests-table bulk bar. PR #16, **merged 2026-09-15**. This row previously read "open and unmerged — do not merge or extend until the planner has read this spec"; that was true when written (see `docs/HANDOFF.md` session 11, where it was built before the spec had been read) and stale from the day it merged. Corrected session 29. | V1 only |
...
| 8 | [Vendor management — contacts, notes, and the budget link](08-vendor-management.md) | **Built end to end, session 29 (2026-09-21)** — all ten questions answered and the build authorized the same day. `0029_vendors.sql`: `vendor_stage`, `vendor_categories`, `vendors`, `vendor_contacts`, `vendor_notes`, the one-primary partial unique index, `budget_items.vendor_id`, `run_sheet_items.vendor_id`, `v_vendors`, and `v_budget_items`/`v_reminders_due` redefined so a rename reaches /budget and the digest with no sync step. `/vendors`, `/vendors/[id]`, `/vendors/contact-sheet`, `VendorPicker` on /budget, and the §4 backfill (exact-string only — the trigram matcher stays deliberately unused). **Question 1 was answered against the recommendation**: vendor categories are their own taxonomy, not `budget_categories`; `0029` seeds them from the budget's names so the two lists start aligned, and nothing syncs them after. 608 tests, 424 SQL assertions, clean build. **Never opened in a browser**; no category editor UI and no run-sheet vendor UI yet. See the spec's "Build status" for the six things that changed because this spec predates four later ones. | Spec 6 / 6.1, already built |
...
| 16 | [List appearance, sidebar cleanup, and budget links at the section level](16-list-appearance-sidebar-and-budget-section-links.md) | **Built end to end** — `0017_budget_section_links.sql` (`budget_item_sections`, `v_budget_item_tasks.link_source` ranking direct > via_list > via_section, `v_timeline_items.list_icon`); the sidebar's Move up/down replaced by a live outstanding-item count (`getOpenItemCounts`); `lists.color`/`lists.icon` made mutually exclusive everywhere a list's identity renders; the linked-tasks popup gained "Link a section…". **This row read "Proposed, not built" until session 29** — a table saying a shipped feature is unbuilt is how it gets built twice. See the spec's own "Status: built" line. | Spec 3, spec 6 / 6.1, spec 7, spec 12, already built |
...
| 19 | [Budget — an overall budget, percentage allocations, and allocation-derived estimates](19-budget-allocation-percentages.md) | **Built end to end, session 23 (2026-09-20)** — all six questions answered and the build authorized the same day. `0020_budget_allocations.sql` (`weddings.total_budget`, `allocation_pct` on categories and items, `v_budget_category_totals`, both existing budget views recreated), the allocation math in `src/lib/budget.ts`, `src/lib/budget-allocations.ts`, `setTotalBudget`/`setCategoryAllocation`/`applySuggestedAllocations`, and the `/budget` header, category rollup, item allocation field and dashboard stat. 473 tests, 250 SQL assertions, clean build. Same live/browser caveat as every session since 12 — see §13. **§8's suggested percentages shipped as the placeholder they are described as; the numbers still want the planner's eye.** **Corrected 2026-09-20 (§14, `0021_budget_zero_is_not_a_figure.sql`): a zero in estimated/quoted/contracted was outranking real figures in the fallback ladder** — a line quoted at $7,700 with a typed 0 contracted reported $0 current and "100% under its allocation" when it was $2,800 over. Each rung now skips null *or* zero, correcting every total that reads `computed_current` at once. | Spec 6 / 6.1 and spec 18, already built |
| 20 | [Budget — how much of a section's allocation is spoken for, and one estimate column instead of two](20-budget-section-allocation-remaining.md) | **Built end to end, session 24 (2026-09-20)** — all five questions answered and the build authorized the same day. §1–§6: `sectionAllocation` in `src/lib/budget.ts` plus a summary line at the top of every category — what its lines' percentages add to (38+30+42 = 110, not 100), in money, what's left to allocate, and how many lines carry no % — with a live "takes Drinks to 110%" hint in the item editor. §7: spec 19's duplicated "Allocated" and "Estimated" figures collapsed into one Estimate column, the target kept as a secondary note. **No migration and no SQL at all** (§11 decision 3) — a pure function over rows `/budget` already loads. 481 tests, clean build. Both halves are presentation only and have never been looked at in a browser. | Spec 19, built but unmerged |
| 20b | [A last name column and real-name "Side" on the guest list](20-guest-last-name-column-and-named-sides.md) | **Built end to end, same session** — both open questions answered first. `guests.last_name` on the `/guests` grid and a real-name Side dropdown (`partner_a`/`partner_b` resolved to the owner/non-owner collaborator's display name via `sideLabel`). No migration — both columns already existed. **Listed for the first time in session 29:** the file has existed since session 24 with no row here at all, because it shares the number 20 with the budget spec and was overwritten rather than added. Numbered 20b here to keep both findable without renaming a file other documents link to. | V1's `guests.last_name`/`side`; spec 15 for `collaborators.display_name` |
| 21 | [A page of their own — per-household addresses on the wedding site](21-per-household-site-addresses.md) | **Built end to end, session 26 (2026-09-20)** — all eight questions answered and the build authorized the same day. `0022_household_slugs.sql` (`households.slug` + `slug_suffix`, the derivation functions and insert trigger, `household_slug_aliases`, `events.guest_note`, `v_households` extended); `/w/<wedding>/okonkwo-4f7ak` rendering the household's own page in the site theme; the address panel on `/households/[id]`; the per-event guest note; and `/i/[token]` + `/rsvp/[token]` retired into 308s with every sender, the QR route and both print sheets switched to the new URL. The readable half is editable and aliased; the five-character suffix is the credential, redrawn only by `reissueInvitation`. 513 tests, 304 SQL assertions, clean build. **Never opened in a browser and never run against the live project — `0022` still needs applying.** See the spec's "Build status". | Spec 14, built; V1's `households` / `invitations` / `invitation_events` |
...
| 24 | [The builder, as a tool you can work in](24-builder-editing-feel.md) | **Partly built, session 30 (2026-09-22) — from a design handoff, not from this spec, and its five questions are still open.** The planner brought in a set of HTML prototypes for the guest site and `/site` and asked for them to be implemented. What landed of §24: `/site` is now a rail of controls (names & date, cover, theme, palette, typography, chapters, block inspector) beside a live preview at `minmax(300px,25rem) minmax(0,1fr)`, with theme/palette/typography folded in from `/site/theme` and custom hex dropped from the editor. **Not** built: autosave, preview scroll retention, starter content for a new block, block thumbnails, the phone preview toggle. The preview is still the `/site/preview` iframe remounted by `previewKey` — one renderer, never two. The same session built out the Editorial theme (hero, three-column itinerary, section rail, photograph block background, `page_break` band, a household-level Yes/No pair on the RSVP form, two type pairings) and `0030_gift_funds.sql`. 633 tests, 437 SQL assertions, clean build. **Never opened in a browser — and this session is almost entirely type, colour and layout.** See `docs/HANDOFF.md` session 30 for the two scope calls and the two things in the design that were deliberately not adopted. | Spec 23, built |
| 25 | [Blocks that know things](25-blocks-that-know-things.md) | **Built end to end, session 29 (2026-09-21)** — all seven questions answered and the build authorized the same day. Written from fifteen screenshots of a competitor's live site. **Part A:** `0026` — dress codes as records (`dress_codes` + `dress_code_notes` + `events.dress_code_id`) rendering twice, a tag inside the event and a full entry in the attire section whose coverage line is a reverse lookup; `coach_runs.event_id` (with `v_coach_runs` recreated to carry it) so a shuttle renders under the event it serves; `arrival_points` and four nullable columns on `transport_options`, reopening `0017`'s "no cost and no duration" on its own destination-wedding terms; the hero absorbs the countdown; computed `NN · LABEL` eyebrows. **Part B:** `0027` — the guestbook and the public song list, both reopened on one rule, `canPublishImmediately()`: a household's private link publishes, the shared page queues. `song_votes.household_id` is NOT NULL, so "a vote needs identity" is a constraint rather than an intention. **Part C:** `0028` pins every existing wedding to Script so the code default could move to Editorial without restyling anybody; Fraunces + Inter self-hosted, three type roles. 582 tests, 388 SQL assertions, clean build. **Never opened in a browser — and Part C is typography.** See the spec's "Build status" for the six things the build corrected. | Specs 14, 21, 22, 23, all built |
...
**Spec 24 sits on spec 23 and adds nothing to the database.** It is the
editing-feel half of the same feature: spec 23 built the right content model
and shipped a builder whose preview scrolls to the top on every keystroke, whose
new blocks arrive empty, and whose style controls offer the planner a choice
between `paper`, `tinted` and `ink`. Five of its six findings are a line or two
of code, which is the case for doing it now rather than filing it as polish.
**It is also the spec most exposed to this project's standing caveat** — every
problem in it was read off the source rather than off a screen, because nothing
in spec 23 has ever been opened in a browser, and its §2 says so rather than
...
registry with a softer name and spec 14 Q6 cut that twice. Built the same
session; its "Build status" records six things the build corrected, of which
two are worth reading before touching the theme — an undefined CSS custom
property invalidates a whole `font-family` declaration rather than falling
through to the next entry, and the theme default could not simply be flipped
because every wedding that never opened the theme editor was rendering Script
by fallback rather than by choice.
