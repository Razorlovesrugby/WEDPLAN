...
| 15 | [List content — inline editing everywhere, notes sections, calculated due dates, and hiding completed tasks](15-list-content-editing-and-calculated-dates.md) | **Built, same session.** `0016_list_content_and_calculated_dates.sql` (`collaborators.display_name`, `list_sections.kind`, `list_items.due_date_offset_days`); inline title editing on sections/tasks/sub-tasks; a "Names" editor in `/settings`; a due-date × clear button; a fixed/relative date toggle recomputed on wedding-date change; notes-kind sections (covers "research under a section" and "a brain-dump section" with one mechanism); "Hide completed" on `/lists/[id]` and every smart view. `typecheck`, `npm test` (384), `verify-migrations.sh` (192 assertions), `verify-bootstrap.sh`, `npm run build` all pass; not opened in a browser. | Spec 1, spec 10, spec 11, already built |
| 16 | [List appearance, sidebar cleanup, and budget links at the section level](16-list-appearance-sidebar-and-budget-section-links.md) | **Built, same session.** `0017_budget_section_links.sql` adds `budget_item_sections` and reshapes `v_budget_item_tasks`'s `linked_via_list` into a three-way `link_source`; the sidebar's Move up/down buttons are replaced by a live outstanding-task count; `lists.color`/`lists.icon` render mutually exclusive everywhere a list's identity shows (icon wins when set); the budget popup gained "Link a section…" and a section-heading badge on `/lists/[id]`. `typecheck`, `npm test` (384), `verify-migrations.sh` (198 assertions), `verify-bootstrap.sh`, `npm run build` all pass; not opened in a browser. | Spec 3, spec 6 / 6.1, spec 7, spec 12, already built |
