...
| 21 | [A page of their own — per-household addresses on the wedding site](21-per-household-site-addresses.md) | **Proposed, not built — no migration, no route, no screens.** A readable per-household address (`/w/<wedding-slug>/<household>`) replacing the opaque `/rsvp/<token>` as the door to the personalised page, minted from the household screen and managed in the Site tab. Most of the page already exists as `/rsvp/[token]`; §3 is the real decision — a guessable URL over the guest list is a change of credential, not a change of route, and it reopens spec 14 Q1. Nothing is built, schema included, until §9's eight questions are answered. | Spec 14, built; V1's `households` / `invitations` / `invitation_events` |
