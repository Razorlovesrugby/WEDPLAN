...
| 20 | [A last name column and real-name "Side" on the guest list](20-guest-last-name-column-and-named-sides.md) | **Built end to end, session 24 (2026-09-20)** — both open questions answered and the build authorized the same session. No migration — `guests.last_name`/`guests.side` already existed. `sideLabel()` (`src/lib/format.ts`) labels a guest's side with the collaborator names from spec 15 instead of "Partner A"/"Partner B"; `GuestsTable` split "Name" into First/Last name columns plus a new Side badge; `FilterBar` gained the Side filter that `GuestFilters` already supported but had no control for. `partner_a`/`partner_b` map to owner/non-owner collaborator role; Side is a label only and does not change reminder or RSVP-notification routing. 483 tests, clean typecheck and build. | V1 (guests), spec 15 (`collaborators.display_name`), already built |
