...
| 22 | [Inviting per event, answering, and knowing they looked](22-per-event-invite-status-and-tracking.md) | **Proposed, not built — no migration, no screens.** Per-event invite status in `/guests`: household default with per-guest overrides (`guest_event_overrides` + `v_guest_event_invites`), "invite sent" as a real state, clickable cells with a menu rather than a cycle, an event's details shown only to people invited to it (which revises spec 14 §6's list-and-mark rule), one-tap Yes/No from the invitation email, and every open logged rather than only the first. The shape is decided; §10's six questions remain and three of them change what a screen says. | V1's guest list, spec 14, spec 21, all built |
| 23 | [The site builder: blocks, widgets, photos and a live preview](23-site-builder-and-widgets.md) | **Proposed, not built — no migration, no screens.** The larger half of the same request as spec 22. A block builder with a live preview (not a free canvas — it would need a hand-built layout per breakpoint), blocks replacing the twelve fixed sections, draft-then-publish via `site_revisions`, one built page personalised in place, and four widget families: photos (the biggest gap — there is no upload anywhere today), music, map/travel, and dress code / FAQ / countdown. §11 has eight questions; 1 (third-party embeds) and 6 (one page or several) are structural. | Spec 14, spec 21, built; spec 22 supplies the who-sees-what rule |
