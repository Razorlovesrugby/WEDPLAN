...
| 19 | [Budget — an overall budget, percentage allocations, and allocation-derived estimates](19-budget-allocation-percentages.md) | **Proposed, not built — no migration, no screens.** Top-down budgeting on top of spec 6's bottom-up model: `weddings.total_budget`, an `allocation_pct` on both `budget_categories` and `budget_items`, a per-category rollup of target vs. current with the over/under in dollars and percent, and a line's allocation standing in as its estimate until one is typed. Nothing is built, schema included, until its §9 Open Questions are answered — questions 1 and 4 decide what the headline over/under number means. | Spec 6 / 6.1 and spec 18, already built |
