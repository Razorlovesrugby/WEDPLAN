...
| `/budget` | **Per line** (`budget-item-fields.tsx`, `budget-item-row.tsx`): an "Allocation %" field next to the estimate, with a live hint under it — "10% of Drinks ($3,200) = $320" — and, when the estimate field is empty, the estimate showing that derived figure greyed with an "from allocation" marker rather than blank. **Superseded in part by spec 20 §7:** the separate "Allocated" figure this row description adds was the same number printed twice on a line with nothing typed, and is now one Estimate column with the target as a secondary note. The row shows target vs. current and its variance the same way the category header does. When the category has no percentage or the wedding has no overall budget, the field is still editable but the hint reads "Set a % on Drinks to turn this into an amount" — allocations entered in any order eventually resolve. |
